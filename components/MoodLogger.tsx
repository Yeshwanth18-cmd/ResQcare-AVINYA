...
const MoodLogger: React.FC<MoodLoggerProps> = ({ onLogSuccess }) => {
    ...
    return (
        <div>
            <h3 className="text-lg font-semibold text-center text-slate-800 mb-2">How are you feeling right now?</h3>
            <p className="text-center text-slate-600 mb-6">Log your mood to track your wellness journey.</p>
            
            {/* Emoji Selector */}
            <div className="mb-6">
                <p className="text-sm font-medium text-slate-700 mb-2 text-center">Select an emoji that fits</p>
                <div className="flex justify-center items-center flex-wrap gap-2">
                    {MOOD_EMOJIS.map((e) => (
                        <button
                            key={e}
                            onClick={() => handleEmojiSelect(e)}
                            className={`p-2 rounded-full transition-transform duration-200 ease-in-out ${emoji === e ? 'bg-blue-200 scale-125' : 'hover:bg-slate-200 hover:scale-110'}`}
            ...
            <div className="mb-6">
                ...
                <div className="flex items-center gap-4">
                    <span className="text-sm text-slate-500">Awful</span>
                    <input
                        id="mood-score"
                        type="range"
                        min="1"
                    ...
                    />
                    <span className="text-sm text-slate-500">Great</span>
                </div>
            ...
            {/* Notes Textarea */}
            <div className="mb-6">
                <label htmlFor="mood-notes" className="block text-sm font-medium text-slate-700 mb-2">Add a note (optional, private)</label>
                <textarea
                    id="mood-notes"
                    value={notes}
