...
const EmergencyCard: React.FC<{ contacts: string[] | null }> = ({ contacts }) => {
  return (
    <div className="mt-6 p-4 themed-alert-error rounded-r-lg flex items-start gap-4">
        ...
        {contacts && contacts.length > 0 && (
           <div className="mt-3 flex flex-wrap gap-3">
               {contacts.map(contact => (
                   <a key={contact} href={`tel:${contact}`} className="inline-flex items-center gap-2 themed-bg-error themed-bg-error-hover text-white font-bold py-2 px-4 rounded-full transition-colors text-sm">
