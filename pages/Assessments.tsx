...
const AssessmentCard: React.FC<{ to: string; title: string; description: string; }> = ({ to, title, description }) => (
  <Link to={to} className="block bg-white p-8 rounded-lg shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
    <h3 className="text-2xl font-bold text-primary">{title}</h3>
    <p className="mt-2 text-slate-600">{description}</p>
    <div className="mt-6 text-primary-text font-semibold">
      Start Screener &rarr;
    </div>
  </Link>
);
